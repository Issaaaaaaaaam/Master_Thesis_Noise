...
def split_log_by_protocol(lines):
    ...
    setup_pattern = re.compile(r'SETUP: (Receiver|Initiator)_(Noise_[^\s]+)')

    for line in lines:
        match = setup_pattern.search(line)
        if match:
            current_key = f"{match.group(1)}_{match.group(2)}"
        if current_key:
            pattern_map[current_key].append(line)

    return list(pattern_map.items())
...
def main():
    ...
    segments = split_log_by_protocol(clean_lines)

    print(f"Found {len(segments)} protocol segments.")

    for setup_info, segment_lines in segments:
        print(f"\nProcessing segment: {setup_info}")

        role, pattern = setup_info.split("_", 1)
        all_benchmarks = parse_benchmarks(segment_lines)

        if not all_benchmarks:
            print(f"No benchmarks found for {setup_info}")
            continue
        ...
        with open(copied_log, 'w', encoding='utf-8') as copy:
            for line in segment_lines:
                clean_line = remove_ansi_codes(line)
                copy.write(clean_line + "\n")

        print(f"Finished: {setup_info}")
