...
#include "crypto/kyber_768/api.h"
#include <time.h>
...
// Create OQS_KEM object initialized with Kyber-768.

typedef struct {
    struct NoiseDHState_s parent;
    uint8_t private_key[PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES];
        ...
        (NoiseDHState *state, const NoiseDHState *other)
{
    ...
    PQCLEAN_MLKEM768_CLEAN_crypto_kem_keypair(st->public_key, st->private_key);
...
static int noise_kyber_set_keypair(NoiseDHState *state, const uint8_t *sk, const uint8_t *pk) {
    NoiseKyberState *st = (NoiseKyberState *)state;
    memcpy(st->private_key, sk, PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES);
    memcpy(st->public_key, pk, PQCLEAN_MLKEM768_CLEAN_CRYPTO_PUBLICKEYBYTES);
    return NOISE_ERROR_NONE;
}
        ...
        (NoiseDHState *state, const NoiseDHState *from, const NoiseDHState *other)
{
    ...
    memcpy(st->private_key, from_st->private_key, PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES);
    memcpy(st->public_key, from_st->public_key, PQCLEAN_MLKEM768_CLEAN_CRYPTO_PUBLICKEYBYTES);
    return NOISE_ERROR_NONE;
}
...
static int noise_kyber_encapsulate(const NoiseDHState *state, uint8_t *ct, uint8_t *ss) {
    ...
    bench_start_kyber("Kyber768 Encapsulation");
    PQCLEAN_MLKEM768_CLEAN_crypto_kem_enc(ct, ss, st->public_key);
    bench_end_kyber("Kyber768 Encapsulation");
    return NOISE_ERROR_NONE;
}

static int noise_kyber_decapsulate(const NoiseDHState *state, const uint8_t *ct, uint8_t *ss) {
    const NoiseKyberState *st = (const NoiseKyberState *)state;
    bench_start_kyber("Kyber768 Decapsulation");
    PQCLEAN_MLKEM768_CLEAN_crypto_kem_dec(ss, ct, st->private_key);
    bench_end_kyber("Kyber768 Decapsulation");
    return NOISE_ERROR_NONE;
}
NoiseDHState *pqnoise_kyber_768_new(void)
{
    ...
    if (!state)
        return 0;
    state->parent.dh_id = NOISE_DH_KYBER_768;
    state->parent.nulls_allowed = 0;
    state->parent.private_key_len = PQCLEAN_MLKEM768_CLEAN_CRYPTO_SECRETKEYBYTES;
    state->parent.public_key_len = PQCLEAN_MLKEM768_CLEAN_CRYPTO_PUBLICKEYBYTES;
    state->parent.shared_key_len = PQCLEAN_MLKEM768_CLEAN_CRYPTO_BYTES;
    state->parent.cipher_len = PQCLEAN_MLKEM768_CLEAN_CRYPTO_CIPHERTEXTBYTES;
